...
void Game::createPlayer() {
	...
	player_render->Create(system, player, &game_objects, "data/crosshair.bmp", 32);
	player->Create();
...
void Game::createRockets() {
	...
	for (auto rocket = rockets_pool.pool.begin(); rocket != rockets_pool.pool.end(); rocket++) {
		...
		render->Create(system, *rocket, &game_objects, "data/rocket.bmp", 32);
		(*rocket)->Create();
...
void Game::createMissiles() {
	...
	for (auto alien = aliens_pool.pool.begin(); alien != aliens_pool.pool.end(); alien++) {
		...
		alien_render->Create(system, *alien, &game_objects, "data/enemy_0.bmp", 32);
	...
	for (auto bomb = bombs_pool.pool.begin(); bomb != bombs_pool.pool.end(); bomb++) {
		...
		bomb_render->Create(system, *bomb, &game_objects, "data/bomb.bmp", 32);
...
void Game::createCities() {
	...
	for (auto city = cities_pool.pool.begin(); city != cities_pool.pool.end(); city++) {
		RenderComponent * city_render = new RenderComponent();
		city_render->Create(system, *city, &game_objects, "data/enemy_1.bmp", 32);
		CollideComponent * city_coll = new CollideComponent();
		city_coll->Create(system, *city, &game_objects, (ObjectPool<GameObject>*)&rockets_pool);
		(*city)->Create();
