...
void PlayerBehaviourComponent::Update(float dt) {
	...
	if (keys.right) MoveRelative(dt * PLAYER_SPEED, 0);
	if (keys.left) MoveRelative(-dt * PLAYER_SPEED, 0);
	if (keys.up) MoveRelative(0, -dt * PLAYER_SPEED);
	if (keys.down) MoveRelative(0, dt * PLAYER_SPEED);
	if (keys.fire) {
		if (CanFire()) {
