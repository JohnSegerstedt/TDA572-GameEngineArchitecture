...
void Game::Create(AvancezLib* system) {
	...
	enemies.insert((ObjectPool<GameObject>*) &missile_pool);
	enemies.insert((ObjectPool<GameObject>*) &bomb_pool);
	enemies.insert((ObjectPool<GameObject>*) &bomber_pool);
	createPlayer();
...
void Game::Init() {
	...
	audio_system->getInstance()->PlaySound(THEME);
...
void Game::createRockets() {
	...
	for (auto rocket = rockets_pool.pool.begin(); rocket != rockets_pool.pool.end(); rocket++) {
		...
		rocket_coll->Create(system, *rocket, &game_objects, &enemies);
		(*rocket)->Create(2);
...
void Game::createMissilesSystem() {
	//system->init_ai(&game_objects, &missile_pool, &bomber_pool, &cities_pool, &level, this);
...
void Game::createCities() {
	...
	for (auto city = cities_pool.pool.begin(); city != cities_pool.pool.end(); city++) {
		...
		city_coll->Create(system, *city, &game_objects, &enemies);
		(*city)->Create(1);
...
void Game::createExplosions() {
	...
	for (auto explosion = explosion_pool.pool.begin(); explosion != explosion_pool.pool.end(); explosion++) {
		...
		explosion_coll->Create(system, *explosion, &game_objects, &enemies);
