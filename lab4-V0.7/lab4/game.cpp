...
void Game::Restart() {
	/*for (auto rocket = rockets_pool.pool.begin(); rocket != rockets_pool.pool.end(); rocket++)		(*rocket)->enabled = false;
	for (auto city = cities_pool.pool.begin(); city != cities_pool.pool.end(); city++)				(*city)->enabled = false;
	for (auto missile = missile_pool.pool.begin(); missile != missile_pool.pool.end(); missile++)	(*missile)->enabled = false;
	for (auto bomber = bomber_pool.pool.begin(); bomber != bomber_pool.pool.end(); bomber++)		(*bomber)->enabled = false;
	for (auto bomb = bomb_pool.pool.begin(); bomb != bomb_pool.pool.end(); bomb++)					(*bomb)->enabled = false;
...
bool Game::UpdateGame(float dt) {
	...
	case GAME_OVER:
		dt = 0.f;
		system->drawText(SCREEN_WIDTH * 1.77 / 4.0, SCREEN_HEIGHT * 1.0 / 4.0, "GAME OVER");
		system->drawText(SCREEN_WIDTH * 1.65 / 4.0, SCREEN_HEIGHT * 1.2 / 4.0, "PRESS TO RESTART");
		if (keys.sprint || keys.right || keys.left || keys.up || keys.down) {
			system->terminate();
			return true;
		}
		break;
	case STARTUP:
		dt = 0.f;
		system->drawText(SCREEN_WIDTH * 1.7 / 4.0, SCREEN_HEIGHT * 1.0 / 4.0, "PRESS TO START");
		if (keys.sprint || keys.right || keys.left || keys.up || keys.down) game_state = PLAYING;
		break;
	case PAUSED:
		dt = 0.f;
		system->drawText(SCREEN_WIDTH * 1.7 / 4.0, SCREEN_HEIGHT * 1.0 / 4.0, "GAME PAUSED");
		if (keys.sprint || keys.right || keys.left || keys.up || keys.down) game_state = PLAYING;
		break;
	case PLAYING:
		if (keys.pause) game_state = PAUSED;
		break;
	}

	for(int i=0; i < NUM_LAYERS; i++) for (auto go = game_objects.begin(); go != game_objects.end(); go++) if((*go)->depth == i)(*go)->Update(dt);
...
void Game::Draw() { // todo, positions on screen is hardcoded here
	...
	system->drawText(SCREEN_WIDTH*0.47f, 32, msg);
...
void Game::Receive(Message m) {
	...
	if (m == MISSILE_HIT) {
		...
		if (--cities_alive == 0) game_state = GAME_OVER;
	}
