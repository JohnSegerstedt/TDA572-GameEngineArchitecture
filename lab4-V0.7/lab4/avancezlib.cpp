	...
	while (SDL_PollEvent(&event)) {
		...
		if (event.type == SDL_KEYDOWN || SDL_KEYUP) {
			...
			// Put this outside the switch case since switches cannot do '||'...
					if (event.key.keysym.sym == UP_1	|| event.key.keysym.sym == UP_2)	key.up = event.type == SDL_KEYDOWN;
			else	if (event.key.keysym.sym == DOWN_1	|| event.key.keysym.sym == DOWN_2)	key.down = event.type == SDL_KEYDOWN;
			else	if (event.key.keysym.sym == LEFT_1	|| event.key.keysym.sym == LEFT_2)	key.left = event.type == SDL_KEYDOWN;
			else	if (event.key.keysym.sym == RIGHT_1 || event.key.keysym.sym == RIGHT_2) key.right = event.type == SDL_KEYDOWN;
			...
			case FIRE:
				...
				break;
			case PAUSE:
				key.pause = event.type == SDL_KEYDOWN;
				if(key.pause) std::cout << "test" << "\n";
				break;
			case SPRINT:
