...
public:
	...
	virtual void Init() {
		int missileWait = 0;
		missiles_num = std::min(40.0, (rand() * (*level) * 5.0 / (float)RAND_MAX) + 10);
		for (int i = 0; i < missiles_num; i++) {
			missile_wait[i] = std::max(0, int(rand() * std::max(2, int(10 - *level)) / (float)RAND_MAX));
		...
		for (int i = 0; i < bomber_num; i++){
			bomber_wait[i] = std::max(2, int(rand() * std::max(1, int(10 - (*level))) / (float)RAND_MAX));
	...
	virtual void Update(float dt) {
		...
		if (bomber_index < bomber_num) {
			...
			if (timeSinceLastBomberLaunch > bomber_wait[bomber_index]) {
				std::cout << timeSinceLastBomberLaunch << "\n";
